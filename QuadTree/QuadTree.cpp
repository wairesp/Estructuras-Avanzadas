...
public:
    QT(int h, int x0, int x1, int y0, int y1) {
    ...
    bool _delete(MyVec input) {
        if (hijos[0] == NULL) {
            for (int i = 0; i < data.size(); ++i) //i<data.size()
                if (data[i]->x == input.x && data[i]->y == input.y) {
                    auto toDelete = data[i]; // agarra la referencia
                    data.erase(data.begin() + i); // borra del vector
        ...
        /// no es hoja
            ...
            int cuadrante = getCuadrante(input.x, input.y);
            return hijos[cuadrante]->find(input);
        }
    }

    bool find(MyVec input) {
        if (hijos[0] == NULL) {
            for (auto el : data) //i<data.size()
                if (el->x == input.x && el->y == input.y)
                    return true;
            return false;
        }
        /// no es hoja
...
int main()
{
    ...
    test->print();
    //std::cout << '\n';
    //std::cout << test->find(MyVec(2, 3)) << '\n';
    //std::cout << test->_delete(arr[0]);
